...
function ResumoConsumo() {
  ...
  // bonus_gb vem do banco (admin libera GB extras manualmente)
  const bonusDebito = baseLine.bonusGb ?? 0;
  // Se total_gb for 0 (scraper ainda nao preencheu), usa um default para evitar divisao por zero
  const safeTotal = baseLine.total > 0 ? baseLine.total : 130;
    ...
    <div className="min-h-screen bg-[#f3f3f3]">
      ...
      <main className="mx-auto max-w-[1400px] px-2 pt-6 pb-16 md:px-6 md:pt-8">
        ...
        <section className="mt-6">
          <div
            className="overflow-hidden rounded-md p-3 pb-10 md:px-9 md:py-6 md:pb-9"
            style={{
      ...
      {(() => {
        const currentStatus: LineStatus = baseLine.status;
        const cfg = ({
          aguardando: {
            label: "Aguardando",
            tone: "#6B7280",
          ...
          reduzida: {
            ...
            fatura: "Em dia",
            message:
              "Sua franquia foi totalmente consumida, e a navegação seguirá em velocidade reduzida até a próxima renovação do ciclo.",
          },
          bloqueada_fatura: {
